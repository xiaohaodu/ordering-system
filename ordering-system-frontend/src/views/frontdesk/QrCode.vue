<template>
    <canvas id="canvas" ref="canvas" class="canvas" :width="props.width" :height="props.height"></canvas>
    <div class="qrcode" :style="{ width: props.width + 'px' }">
        <el-input type="number" :change="handleQrcode()" v-model="props.qrParams" placeholder="请输入桌号" />
        <div @click="savePic" style="cursor: pointer;display: flex;align-items: center;justify-content: center;">
            <span>下载二维码</span>
            <el-icon>
                <Download />
            </el-icon>
...
import { onMounted, ref, reactive, defineProps, getCurrentInstance, ComponentInternalInstance } from "vue";
...
const handleQrcode = async () => {
    ...
    try {
        const url = await QRCode.toDataURL(props.qrUrl + '/' + props.qrParams, qrCodeOption);
...
const savePic = () => {
    if (props.qrParams < 1) {
        return proxy.$elMessage('请输入大于1的正常桌号');
    }
    let dom = canvas.value as HTMLCanvasElement;
