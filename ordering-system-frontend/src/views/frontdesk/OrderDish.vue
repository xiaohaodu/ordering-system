<template>
  <div class="manage" v-for="(type, index) in menu" :key="index">
    ...
    <div class="menu">
      <div class="dish" v-for="(dish, index) in type.dish" :key="index">
        ...
        <div
          class="price"
          v-html="`<span style='color:black'>单价：</span>` + dish.price + '￥'"
        ></div>
        <el-input-number
          v-model="dish.num"
          :min="0"
        ...
        <div
          class="total-price"
          v-html="`<span>总价：</span>` + dish.num * dish.price + '￥'"
        ></div>
      </div>
    </div>
  </div>
  <div class="submit">
    <span
      v-html="
        `<span style='color:black'>订单总价：</span>` + order.TotalPrice + '￥'
      "
    ></span>
...
async function submitOrder() {
  ...
  try {
    ...
    // console.log(res);
    ws.send(
      JSON.stringify({ type: "mobile", isMessage: true, message: order })
    );
...
const handleChange = (currentValue: number, oldValue: number, dish, type) => {
  ...
  for (const key in order.dish) {
    ...
    order.dish[key].num == 0
      ? order.dish.splice(key, 1)
      : (order.TotalPrice += order.dish[key].num * order.dish[key].price);
  }
};
