<template>
    <img class="add-dish" @click="dialogFormVisible = true" src="@/assets/Add.svg" alt="addDish">
    <el-dialog center v-model="dialogFormVisible" title="添加菜单">
        <el-form :model="dish">
            ...
            <el-form-item label="展示图片" :label-width="formLabelWidth">
                <el-upload action="#" v-model:file-list="picture" :limit="1" :on-preview="handlePictureCardPreview"
        ...
        <template #footer>
            <span class="dialog-footer">
                ...
                <el-button type="primary" @click="addDish(dish)">
                    确定
                </el-button>
            </span>
        </template>
...
import { reactive, ref, toRefs, getCurrentInstance, defineEmits } from 'vue'
    ...
    try {
        if (picture.value.length > 0) {
            ...
            base.onload = async () => {
                ...
                emit('getMenu')
                proxy.$elMessage(res.message, res.status == 0 ? 'success' : 'error')
            }
        } else {
            proxy.$elMessage('请传入完整信息！')
        }
    } catch (error) {
